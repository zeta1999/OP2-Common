...
def op_par_loop_parse(text):
    ...
    search = "op_par_loop"
    i = text.lower().find(search.lower())
    ...
    while i > -1 and i1 > -1:
      ...
      if j > -1 :
        ...
        j = text2.lower().rfind('program'.lower(), 0, i1)
        if j > -1:
          pattern = re.compile('\n')
          s = pattern.search(text2[j+7:])
          routine =  text2[j+7:j+7+s.start()]
        else:
          print 'ERROR: Loop is not within a subroutine or a program !'
          exit(2)
...
#
  date = datetime.datetime.now() # obtain the date for code generation
...
#OpenMP4 offload
#TODO op2_gen_openmp4(str(sys.argv[init_ctr]), date, consts, kernels, hydra, bookleaf)  # optimised by removing the overhead due to fortran c to f pointer setups
