    ...
    if hydra == 1:
      ...
      for g_m in range(0,nargs):
        if var[g_m] == 'npdes':
          using_npdes = 1
      if using_npdes:
        # search for the first occurnace of npdes
        # as it is the integer declaration of it
...
##########################################################################
    if hydra:
      ...
      #filename = kernels[nk]['master_file'].upper()+kernels[nk]['mod_file'].upper()+name+'_seqkernel.F90'
      filename = kernels[nk]['mod_file'].upper()+'_seqkernel.F90'
      #print filename
      fid = open('mpi/'+filename,'w')
    elif bookleaf:
