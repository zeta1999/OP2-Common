...
%
  ...
  file = strvcat(file,...
    '                                                                  ',...
    '#ifdef CHECKPOINTING                                              ',...
    '  if(op_checkpointing_name_before(args,nargs,name)) {             ',...
    '#endif                                                            ',...
    '                                                                  ',...
    '  // consistency checks                                           ',...
    '                                                                  ',...
  ...
  for n = 1:nargs
    file = strvcat(file,['    op_arg_set(n, arg',num2str(n-1),', &p_a[',num2str(n-1),'], halo);                     ']);
  end
...
%
  ...
  file = strvcat(file,...
     ...
     ['  op_mpi_perf_comms(k_i, ' c_nargs ', args);'],...
      ...
      '#ifdef CHECKPOINTING',...
      '  }',...
      '  op_checkpointing_name_after(args,nargs,name);',...
      '#endif');
