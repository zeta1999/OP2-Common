    ...
    if ninds>0:
      ...
      if nmaps > 0:
        ...
        for g_m in range(0,nargs):
          if maps[g_m] == OP_MAP and (not mapinds[g_m] in k):
            k = k + [mapinds[g_m]]
            if has_doubles:
              code('intv_half map'+str(mapinds[g_m])+'idx(&arg'+str(invmapinds[inds[g_m]-1])+'.map_data_d['+str(vector_size)+'*n + set_size * '+idxs[g_m]+']);')
            else:
              code('intv map'+str(mapinds[g_m])+'idx(&arg'+str(invmapinds[inds[g_m]-1])+'.map_data_d['+str(vector_size)+'*n + set_size * '+idxs[g_m]+']);')
      code('')
      #
      # Gather
      #
      ...
      for g_m in range(0,nargs):
        ...
        if maps[g_m] == OP_MAP and accs[g_m] <> OP_INC:
          ...
          indent = '\n'+' '*(depth+2)
          for d in range(0,int(dims[g_m])):
            line = line + indent + vectyps[g_m]+'(('+typs[g_m]+'*)arg'+str(g_m)+'.data, '+mapidx+', '+str(d)+')'
            if d < int(dims[g_m])-1:
              line = line +','
            else:
                line = line +'};'
        ...
        if maps[g_m] == OP_GBL and accs[g_m]==OP_READ:
          code(vectyps[g_m]+' ARG'+idxs[g_m]+'(('+typs[g_m]+'*)arg'+str(g_m)+'.data);')
      ...
      for g_m in range(0,nargs):
        ...
        if maps[g_m] == OP_MAP and accs[g_m] <> OP_READ:
          ...
          if accs[g_m] == OP_INC:
            kind = '_add'
          for d in range(0,int(dims[g_m])):
            code('store_scatter'+kind+'(ARG_p['+str(d)+'], ('+typs[g_m]+'*)arg'+str(g_m)+'.data, '+mapidx+','+str(d)+');')
        if maps[g_m] == OP_GBL and accs[g_m]==OP_INC:
