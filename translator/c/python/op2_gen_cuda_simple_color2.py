...
def op2_gen_cuda_simple_color2(master, date, consts, kernels,sets):
...
#
    if ninds>0:
      ...
      if ind_inc:
        code('int idx = (blockIdx.x+blockIdx.y*gridDim.x)*blockDim.x + threadIdx.x;')
        IF('idx<nelems')
        code('int n = elements[idx];')
      else:
