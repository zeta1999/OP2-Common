...
void op_x86_res_calc(
  ...
  if (0==0) {
    ...
    nbytes    += ROUND_UP(ind_arg1_size*sizeof(float)*1);
  ...
  for (int n=0; n<nelem; n++) {
    ...
    for (int d=0; d<1; d++)
      arg9_l[d] = ZERO_float;
    for (int d=0; d<1; d++)
      arg10_l[d] = ZERO_float;
    for (int d=0; d<1; d++)
      arg11_l[d] = ZERO_float;
    for (int d=0; d<1; d++)
      arg12_l[d] = ZERO_float;
...
void op_par_loop_res_calc(char const *name, op_set set,
  ...
  args[4] = arg4;
  for (int v = 1; v < 4; v++) {
    args[4 + v] = op_arg_dat(arg4.dat, v, arg4.map, 1, "float", OP_READ);
  }
  args[8] = arg8;
  arg9.idx = 0;
  args[9] = arg9;
  for (int v = 1; v < 4; v++) {
    args[9 + v] = op_arg_dat(arg9.dat, v, arg9.map, 1, "float", OP_INC);
  }
