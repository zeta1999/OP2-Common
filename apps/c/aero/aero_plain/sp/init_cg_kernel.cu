...
__global__ void op_cuda_init_cg(
  ...
  float arg1_l[1];
  for (int d=0; d<1; d++) arg1_l[d]=ZERO_float;
...
void op_par_loop_init_cg(char const *name, op_set set,
  ...
  if (set->size >0) {
    ...
    for (int b=0; b<maxblocks; b++)
      for (int d=0; d<1; d++)
        ((float *)arg1.data)[d+b*1] = ZERO_float;
    reduct_bytes += ROUND_UP(maxblocks*1*sizeof(float));
    ...
    op_cuda_init_cg<<<nblocks,nthread,nshared>>>( (float *) arg0.data_d,
                                                  (float *) arg1.data_d,
                                                  (float *) arg2.data_d,
                                                  (float *) arg3.data_d,
                                                  (float *) arg4.data_d,
    ...
    for (int b=0; b<maxblocks; b++)
      for (int d=0; d<1; d++)
        arg1h[d] = arg1h[d] + ((float *)arg1.data)[d+b*1];
