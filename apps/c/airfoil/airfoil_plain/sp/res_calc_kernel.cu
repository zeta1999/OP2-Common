...
__global__ void op_cuda_res_calc(
  ...
  for ( int n=threadIdx.x; n<ind_arg1_size*4; n = n+=blockDim.x ){
    ind_arg1_s[n] = ind_arg1[n%4+ind_arg1_map[n/4]*4];

  }
  for ( int n=threadIdx.x; n<ind_arg2_size*1; n = n+=blockDim.x ){
    ind_arg2_s[n] = ind_arg2[n%1+ind_arg2_map[n/1]*1];

  }
  for ( int n=threadIdx.x; n<ind_arg3_size*4; n = n+=blockDim.x ){
