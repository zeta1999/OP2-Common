...
__global__ void op_cuda_bres_calc(
  ...
  for ( int n=threadIdx.x; n<nelems2; n = n+=blockDim.x ){
    ...
    for ( int col=0; col<ncolor; col++ ){
      if (col2==col) {
        arg4_l[0] += ind_arg3[0 * ind0_stride + map2idx];
        arg4_l[1] += ind_arg3[1 * ind0_stride + map2idx];
        arg4_l[2] += ind_arg3[2 * ind0_stride + map2idx];
        arg4_l[3] += ind_arg3[3 * ind0_stride + map2idx];
        ind_arg3[0 * ind0_stride + map2idx] = arg4_l[0];
        ind_arg3[1 * ind0_stride + map2idx] = arg4_l[1];
        ind_arg3[2 * ind0_stride + map2idx] = arg4_l[2];
        ind_arg3[3 * ind0_stride + map2idx] = arg4_l[3];
      }
      __syncthreads();
    }
  }
  ...
  if (set->size > 0) {
    ...
    for ( int col=0; col<Plan->ncolors; col++ ){
      ...
      if (Plan->ncolblk[col] > 0) {
        ...
        arg0.map_data_d,
        arg2.map_data_d,
        arg4.dat->set->size+arg4.dat->set->exec_size+arg4.dat->set->nonexec_size,
        (int*)arg5.data_d,
        block_offset,
        Plan->blkmap,
        Plan->offset,
