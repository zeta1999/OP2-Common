...
{
  char *l_str = UTILS_copyfstr(str, *len);
  
  errno = 0;
  (void) UTILS_error(l_str, NULL);
  (void) free(l_str); l_str = NULL;
  
  return;
}

void F2C(UTILS_pauseverbose_c,UTILS_PAUSEVERBOSE_C)(int *toggle)
{
  /* Disables verbose mode and returns the previous setting in toggle */
...
void F2C(UTILS_resumeverbose_c,UTILS_RESUMEVERBOSE_C)(int *toggle)
{
  /* Sets verbose mode to the toggle value */
...
void F2C(utils_mkdir_c,UTILS_MKDIR_C)(const char *path, const int *pathlen,
			        	int *err)
{
  char *p = UTILS_copyfstr(path, *pathlen);
...
void F2C(utils_ln_c,UTILS_LN_C)(const char *path,      const int *pathlen,
				  const char *link_path, const int *link_pathlen,
				  int *err)
{
  char *p = UTILS_copyfstr(path,      *pathlen);
  char *l = UTILS_copyfstr(link_path, *link_pathlen);
