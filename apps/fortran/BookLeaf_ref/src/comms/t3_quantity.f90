...
!        irc = ty_MemAdd(40500+PhaseID, TYPH_REAL, size(iPQInfo))
...
!     Still in registration. use Linked list
    ...
    if (irc == TYPH_SUCCESS) then
      if (iAddr==iNULL_ADDR) then
        ...
        if (allocated(iQuant%uppbnd)) deallocate(iQuant%uppbnd)
        if (allocated(iQuant%lowbnd)) deallocate(iQuant%lowbnd)
        allocate(iQuant%uppbnd(iQuant%rank))
        allocate(iQuant%lowbnd(iQuant%rank))
        do iDim = 1, iQuant%rank
          iQuant%uppbnd(iDim) = UBOUND(PQuant,DIM=iDim)
          iQuant%lowbnd(iDim) = LBOUND(PQuant,DIM=iDim)
        enddo
      endif
