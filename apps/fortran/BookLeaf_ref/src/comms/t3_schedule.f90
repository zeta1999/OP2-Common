...
module TYPH_Schedule_mod
  ...
  integer(kind=TSIZEK), save :: mMaxCommProc = 0     ! Max procs to send to
  integer(kind=TSIZEK), save :: mMaxParts    = 0     ! Largest SchedulePart size

contains

  integer(kind=TERRK) function ty_BuildSchedule( aPhaseID, NumGhosts )
...
! Routine breakdown is essentially:
    ...
    integer(kind=TSIZEK), intent(in), optional  :: NumGhosts ! Number of ghost layers
                                                             ! requested by Ext
    ...
    integer(kind=TSIZEK), dimension(:),allocatable :: iDims       ! Array of non-mesh dims
    integer(kind=TSIZEK), dimension(:),allocatable :: iScounts    ! No. valid Send keys to each proc
    integer(kind=TSIZEK), dimension(:),allocatable :: iRcounts    ! No. valid Recv keys from each proc
    integer(kind=TSIZEK)                           :: iTcounts    ! Total send + recv counts
    integer(kind=TSIZEK)                           :: iMinLay     ! Minimum Ghost layer
    integer(kind=TSIZEK)                           :: iMaxLay     ! Maximum Ghost layer
    integer(kind=TSIZEK)                           :: iKeyID      ! Key index
    integer(kind=TSIZEK)                           :: iQuantSize  ! Quant Size
    integer(kind=TSIZEK)                           :: iDimProd    ! Product of Quant dimensions
    integer(kind=TSIZEK)                           :: iLayer      ! Current layer
    integer(kind=TSIZEK)                           :: iNSend      ! Number of nodes/cells sent
    integer(kind=TSIZEK)                           :: iNRecv      ! Number of nodes/cells received
    integer(kind=TSIZEK)                           :: iSend       ! Send counter
    integer(kind=TSIZEK)                           :: iRecv       ! Recv counter
    integer(kind=TSIZEK)                           :: iProc       ! Processor counter
    integer(kind=TSIZEK)                           :: iPos        ! Current index in array
    integer(kind=TSIZEK)                           :: iStart      ! Start index
...
!         Work out min/max layer
  ...
  integer(kind=TERRK) function ty_DeleteSchedule( aPhaseID )
...
!   Then deallocate the Schedule itself
  ...
  integer(kind=TERRK) function ty_CommitPhase( aPhase )
...
!    type(V3_comm_Quant_tp),   pointer  :: iPQinfo       => null()
    ...
    integer(kind=TSIZEK)                           :: iCount      ! Counter
    integer(kind=TSIZEK)                           :: iSend       ! Send Counter
    integer(kind=TSIZEK)                           :: iRecv       ! Recv Counter
    integer(kind=TSIZEK)                           :: iProc       ! Processor counter
    integer(kind=TSIZEK)                           :: iAllocStat
    integer(kind=TSIZEK)                           :: iNlist
    integer(kind=TSIZEK)                           :: iList
    integer(kind=TSIZEK)                           :: ii
    integer(kind=TSIZEK)                           :: jj

    integer(kind=MPIK),  dimension(:), allocatable :: iMpiTypeArr      ! MPI Types
    integer(kind=MPIK),  dimension(:), allocatable :: iMpiBlklenArr    ! Blocklengths
