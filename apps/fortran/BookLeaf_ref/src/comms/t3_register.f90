...
module TYPH_Register_mod
  ...
  integer(kind=TYPHK), public, parameter :: TYPH_PURE            = 1

  integer(kind=TYPHK), public, parameter :: TYPH_GHOSTS_NULL     = TYPH_NULL
  integer(kind=TYPHK), public, parameter :: TYPH_GHOSTS_MAX      = 4


  integer(kind=TYPHK), public, parameter :: TYPH_CENTRE_NODE     = 2001
  integer(kind=TYPHK), public, parameter :: TYPH_CENTRE_CELL     = 2002

  type :: Phase_tp
    integer(kind=TSIZEK)       :: nghosts
    integer(kind=TSIZEK)       :: nquants
    logical                    :: pure
    character(len=TYPH_STRLEN) :: name
    integer(kind=TYPHK), dimension(:), allocatable :: quantlist
!   Typhon V3 components
  ...
  type :: Quant_tp
    integer(kind=TSIZEK)       :: qdataID
    integer(kind=TSIZEK)       :: nghosts
    integer(kind=TYPHK)        :: centring
    integer(kind=TYPHK)        :: datatype
    logical                    :: pure, aux
    character(len=TYPH_STRLEN) :: name
...
!   Typhon V3 components
    ...
    integer(kind=TSIZEK)                        :: stride   ! stride across memory for multiD arrays
    integer(kind=TMEMK)                         :: quant_address = TYPH_NULL_ADDR
                                                            ! Start address of quant.
                                                            ! Only set after allocation
    integer(kind=TSIZEK),dimension(:),allocatable:: lowbnd  ! Array Lower boundary
    integer(kind=TSIZEK),dimension(:),allocatable:: uppbnd  ! Array Upper boundary
  end type Quant_tp
...
! Can this be more cleverly with F2003?  
  ...
  integer(kind=TYPHK), parameter :: mLL_QUANT = 40001
  ...
  integer(kind=TSIZEK), save :: mNumPhases = 0
  integer(kind=TSIZEK), save :: mNumQuants = 0
...
contains
  ...
  integer(kind=TERRK) function TYPH_Start_Register()
  ...
  integer(kind=TERRK) function TYPH_Finish_Register()
...
!       For Typhon 3 - ensure Phases need building before use.
  ...
  integer(kind=TERRK) function TYPH_Add_Phase( PhaseID, Name, NumGhosts, PureOrAux,  &
                                               KeySetID, GhostsMin )

    implicit none

    integer(kind=TSIZEK),                      intent(out) :: PhaseID
    character(len=*),                          intent(in)  :: Name
    integer(kind=TSIZEK),                      intent(in)  :: NumGhosts
    integer(kind=TYPHK),                       intent(in)  :: PureOrAux
    integer(kind=TSIZEK),            optional, intent(in)  :: KeySetID
    integer(kind=TSIZEK),            optional, intent(in)  :: GhostsMin
...
!    iPhase%pure = (iand(PureOrAux, TYPH_PURE) == TYPH_PURE)
  ...
  integer(kind=TERRK) function TYPH_Get_Phase( PhaseID, Name, NumGhosts, PureOrAux, &
                                               KeySetID, GhostsMin )

    implicit none
    integer(kind=TSIZEK), intent(in)            :: PhaseID
    character(len=*),     intent(out), optional :: Name
    integer(kind=TSIZEK), intent(out), optional :: NumGhosts
    integer(kind=TYPHK),  intent(out), optional :: PureOrAux
    integer(kind=TSIZEK), intent(out), optional :: KeySetID
    integer(kind=TSIZEK), intent(out), optional :: GhostsMin
  ...
  integer(kind=TERRK) function TYPH_Add_Quant( QuantID, Name, NumGhosts,       &
    ...
    implicit none
    integer(kind=TSIZEK), intent(out) :: QuantID
    character(len=*),     intent(in)  :: Name
    integer(kind=TSIZEK), intent(in)  :: NumGhosts
    integer(kind=TYPHK),  intent(in)  :: Datatype
    integer(kind=TYPHK),  intent(in)  :: Centring
    integer(kind=TYPHK),  intent(in)  :: PureOrAux
...
!    iQuant%pure     = (iand(PureOrAux, TYPH_PURE) == TYPH_PURE)
  ...
  integer(kind=TERRK) function TYPH_Get_Quant( QuantID, Name, NumGhosts,       &
       &                                       Datatype, Centring, PureOrAux,  &
       &                                       AuxID )
    ...
    character(len=*),     intent(out), optional :: Name
    integer(kind=TSIZEK), intent(out), optional :: NumGhosts
    integer(kind=TYPHK),  intent(out), optional :: Datatype
    integer(kind=TYPHK),  intent(out), optional :: Centring
    integer(kind=TYPHK),  intent(out), optional :: PureOrAux
  ...
  integer(kind=TERRK) function TYPH_Tag_Quant( QuantID, PhaseID )
  ...
  integer(kind=TERRK) function mRegisterInit()
  ...
  integer(kind=TERRK) function ty_RegisterKill()
  ...
  integer(kind=TERRK) function ty_GetPhase(aPhaseID, aPhase)
  ...
  integer(kind=TERRK) function ty_GetLLPhase(aPhaseID, aPhase)
  ...
  integer(kind=TERRK) function ty_GetQuant(aQuantID, aQuant)
  ...
  integer(kind=TERRK) function ty_GetLLQuant(aQuantID, aQuant)
...
!   integer(kind=TERRK) function ty_GetNoQuants( aNoQuants )
!     
!     implicit none
!     
!     integer(kind=TSIZEK),     intent(out)  :: aNoQuants
!     
...
!   end function ty_GetNoQuants
  ...
  integer(kind=TERRK) function mGetNameLL(aLLtype, aName, aID)

    implicit none
    integer(kind=TYPHK),  intent(in)  :: aLLtype
  ...
  integer(kind=TERRK) function mKillLL(aLL, aN)
