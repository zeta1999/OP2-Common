...
module TYPH_Key_mod
  ...
  integer(kind=TYPHK), parameter :: TYPH_KTYPE_NODE        = 2
  integer(kind=TYPHK), parameter :: TYPH_KTYPE_CELL_CORNER = 3

  type Key_Set_Array_tp
    type(Key_Set_tp), pointer :: KeySet => null()
  end type Key_Set_Array_tp

  integer(kind=TSIZEK), save                    :: mNKeySets = 0
  type(Key_Set_Array_tp), dimension(:), pointer :: KeySetArray => null()

  integer(kind=TSIZEK), save                    :: mMaxKeySize = 0

  type (Part_Info_tp), pointer :: mPartInfo => null()  ! Partition info
  type (MP_tp),        pointer :: mMP  => null()  ! MPI info

  integer(kind=TSIZEK), dimension(:),   pointer   :: mToGlob       => null()
  integer(kind=TSIZEK), dimension(:,:), pointer   :: mToProc       => null()
  integer(kind=TSIZEK), dimension(:,:), pointer   :: mConnectivity => null()
  integer(kind=TSIZEK), dimension(:), allocatable :: mTotItems ! Total number of ghost cells/nodes
  integer(kind=TSIZEK)                            :: mCentring ! Node/Cell/Corner centring
  integer(kind=TSIZEK)                            :: mStride   ! Stride across memory
...
!   check limits are correct
    if (mCentring == TYPH_CENTRE_NODE) then
      irc = ty_ErrorCheck(((Lmin >= 0_TSIZEK) .and. (Lmin <= mPartInfo%NLayers)) ,    &
            subname, ERR_INT, TYPH_ERR_INT, "Lmin is not between 0 and NGhost_Lay")
      irc = ty_ErrorCheck(((Lmax >= 0_TSIZEK) .and. (Lmax <= mPartInfo%NLayers)) ,    &
            subname, ERR_INT, TYPH_ERR_INT, "Lmax is not between 0 and NGhost_Lay")
    else
      irc = ty_ErrorCheck(((Lmin >= 1_TSIZEK) .and. (Lmin <= mPartInfo%NLayers)) ,    &
            subname, ERR_INT, TYPH_ERR_INT, "Lmin is not between 1 and NGhost_Lay")
      irc = ty_ErrorCheck(((Lmax >= 1_TSIZEK) .and. (Lmax <= mPartInfo%NLayers)) ,    &
...
!   Create a new KeySet to work with
  ...
  integer(kind=TERRK) function mAddKey(aLL, aNkey, aProc, aLayer, aNode)

    implicit none

    type(KeyLL_tp), pointer, intent(inout) :: aLL    ! Head of list
    integer(kind=TSIZEK),    intent(inout) :: aNkey  ! No. keys in list
    integer(kind=TSIZEK),    intent(in)    :: aProc  ! Processor ID
    integer(kind=TSIZEK),    intent(in)    :: aLayer ! Ghost layer
  ...
  integer(kind=TERRK) function mNewKeySet(aID, aCentring, aAuxID, aStride, aLmin,   &
                                          aLmax, aPartitionID, aKeySet)
    implicit none
    integer(kind=TSIZEK),      intent(out) :: aID            ! New KeySet index
    integer(kind=TYPHK),       intent(in)  :: aCentring      ! variable centring (NODE/CELL)
    integer(kind=TSIZEK),      intent(in)  :: aAuxID         ! handle for type of auxilary data
    integer(kind=TSIZEK),      intent(in)  :: aStride        ! no. parts object is divided
                                                             ! into (for cell corner props)
    integer(kind=TSIZEK),      intent(in)  :: aLmin          ! smallest ghost layer index
    integer(kind=TSIZEK),      intent(in)  :: aLmax          ! largest ghost layer index
    integer(kind=TSIZEK),      intent(in)  :: aPartitionID   ! ID of partition used by this KeySet
    ...
    aKeySet%nSend       = 0_TSIZEK
    aKeySet%nRecv       = 0_TSIZEK
  ...
  integer(kind=TERRK) function mAddKeySet(aKeySet, aID)

    implicit none
    type(Key_Set_tp), pointer, intent(in)  :: aKeySet  ! pointer to new KeySet
    integer(kind=TSIZEK),      intent(out) :: aID      ! KeySet index

    type(Key_Set_Array_tp), dimension(:), pointer :: temp_array
    integer(kind=TSIZEK) :: ii
    integer(kind=TERRK)  :: irc
    integer(kind=TYPHK)  :: iAllocStat
    integer(kind=TSIZEK) :: iMemTag
...
! Defines a Key Set
  integer(kind=TERRK) function mDefineKeySet(aPKeySet, aKType, aLmin, aLmax)

    implicit none

    type(Key_Set_tp),     pointer     :: aPKeySet
    integer(kind=TYPHK),  intent(in)  :: aKType        !    Key Type (Node/Cell/Cell Corner)
    integer(kind=TSIZEK), intent(in)  :: aLmin         !    smallest ghost layer index
    integer(kind=TSIZEK), intent(in)  :: aLmax         !    largest ghost layer index

    integer(kind=MPIK),  dimension(MPI_STATUS_SIZE) :: iMpiStatus
    logical(kind=LOGK)                              :: iGlobPresent

    type(KeyLL_tp), pointer :: iPKey => null()

    integer(kind=TSIZEK), dimension(:),   allocatable :: iCounts   ! No. ghost items from each neighb
    integer(kind=TSIZEK), dimension(:),   allocatable :: iTCounts
    integer(kind=TSIZEK), dimension(:),   allocatable :: iStarts
    integer(kind=TSIZEK), dimension(:),   allocatable :: iSendReq
    integer(kind=TSIZEK)                              :: iRecvReq
    integer(kind=TSIZEK), dimension(:,:), allocatable :: iSendItemList
    integer(kind=TSIZEK), dimension(:,:), allocatable :: iRecvItemList

!   counters
    integer(kind=TSIZEK)       :: ii
    integer(kind=TSIZEK)       :: jj
    integer(kind=TSIZEK)       :: kk
    integer(kind=TSIZEK)       :: mm
    integer(kind=TSIZEK)       :: iProc        ! current processor
    integer(kind=TSIZEK)       :: iStartindex  ! start index
    integer(kind=TSIZEK)       :: iLayer       ! current ghost layer

!   sizes
    integer(kind=TSIZEK)       :: iNsize       ! No. properties per communicated item (nd)
    integer(kind=TSIZEK)       :: iNitems      ! No. send items
    integer(kind=TSIZEK)       :: iNrecv       ! No. recv items
    integer(kind=TSIZEK)       :: iNcount      ! message count
    integer(kind=TSIZEK)       :: iNremain     ! remaining message count
    integer(kind=TSIZEK)       :: iSreq        ! send request counter

    character(len=SUBNAME_LEN) :: subname = "mDefineKeySet"
    integer(kind=TERRK)        :: irc
    integer(kind=TYPHK)        :: iAllocStat

    integer(kind=MPIK)         :: iV3_COMM_TAG
...
!   Build key lists for each ghost cell in each ghost layer
    do iLayer = aLmin, aLmax
      ...
      case (TYPH_KTYPE_CELL, TYPH_KTYPE_NODE)
        iNitems = 0_TSIZEK
        if (iLayer > 0) then
          iStartindex = mTotItems(iLayer - 1_TSIZEK) + 1_TSIZEK
        else
          iStartindex = 1_TSIZEK
        endif
...
!       first work out how many ghost cells/nodes belong to each neighbour processor
        do ii = iStartindex, mTotItems(iLayer)
          ...
          iNitems = iNitems + 1_TSIZEK
          iCounts(iProc) = iCounts(iProc) + 1_TSIZEK
        enddo
...
!       Could this be done without the need for the full second loop?
        ...
        do ii = iStartindex, mTotItems(iLayer)
          ...
          iSendItemList(1,iStarts(iProc)) = ii - 1_TSIZEK
          iStarts(iProc) = iStarts(iProc) + 1_TSIZEK   ! Increases index in iSendItemList
        enddo
...
!     we need each corner's component
        ...
        iStartindex = mTotItems(iLayer - 1) + 1_TSIZEK


!       first work out how many ghost cells/nodes belong to each neighbour processor

        jj = mPartInfo%NNod_tot(iLayer-1_TSIZEK)
...
!       Could this be done without the need for the full second loop?
        ...
        do ii = iStartindex, mTotItems(iLayer)
          ...
          do kk = 1, mPartInfo%NodesPerElem
            if (mConnectivity(kk, ii) <= jj) then
              iSendItemList(1,iStarts(iProc)) = (ii-1) * mPartInfo%NodesPerElem + kk - 1_TSIZEK
              iStarts(iProc) = iStarts(iProc) + 1_TSIZEK    ! Increases index in iSendItemList
            endif
          enddo
        enddo
...
!     Fill iSendItemList with global IDs if available
      if (iGlobPresent) then
        do ii = 1, iNitems
          jj = (iSendItemList(1, ii)) / mStride + 1_TSIZEK
...
!     irc = ty_MemAdd(40308, TYPH_INTEGER, iNsize*iNrecv)
      ...
      do iProc = 0, mMP%size - 1_TSIZEK

        if (iCounts(iProc) > 0_TSIZEK) then
...
!         irc = ty_MemAdd(40307, TYPH_INTEGER, iPKey%nlist)
          ...
          ii = 1_TSIZEK
          do jj = iStarts(iProc), iStarts(iProc)+iCounts(iProc) - 1_TSIZEK

!           Build receive key lists for the current processor from each neighbour.
            iPKey%list(ii) = iSendItemList(1,jj)
            mm = (iSendItemList(1, jj)/mStride) + 1_TSIZEK
            kk = iSendItemList(1, jj) - mStride * (mm - 1_TSIZEK) + 1_TSIZEK

!           Convert iSendItemList array to store local displacement on neighbour proc
            iSendItemList(1, jj) = mStride * (mToProc(2, mm) - 1_TSIZEK) + kk - 1_TSIZEK
            ii = ii + 1_TSIZEK

          enddo

!         Send out sections of the iSendItemList to the relevent neighbour proc
          iSreq = iSreq + 1_TSIZEK
...
!         Sanity check. Do the iRecvItemList global IDs match the main global list ones?
          if (iGlobPresent) then
            do ii = 1, iNcount
              jj = (iRecvItemList(1, ii) / mStride) + 1_TSIZEK
...
!   irc = ty_MemRem( 40305, TYPH_INTEGER, mMP%size )
  ...
  integer(kind=TERRK) function ty_GetKey(aKeySetLL, aKeyType, aKeyID, aPKey, aProc, aLayer)

    implicit none

    type(Key_Set_tp), pointer, intent(inout)    :: aKeySetLL ! Head of KeySet Linked list
    integer(kind=TSIZEK),  intent(in)           :: aKeyType  ! Type - Send or recv key
    integer(kind=TSIZEK),  intent(out)          :: aKeyID    ! returned key poition in list
    type(KeyLL_tp), pointer, intent(out)        :: aPKey     ! pointer to returned key
    integer(kind=TSIZEK),  intent(in), optional :: aProc     ! Processor ID
    integer(kind=TSIZEK),  intent(in), optional :: aLayer    ! Ghost layer (default last layer)

    type(KeyLL_tp), pointer      :: iPKey     => null()

    integer(kind=TSIZEK)       :: iCount
    integer(kind=TERRK)        :: irc
...
!
  ...
  integer(kind=TERRK) function ty_GetKeySet(aID, aKeySet)

    implicit none
    integer(kind=TSIZEK),      intent(in)  :: aID       ! KeySet index
    type(Key_Set_tp), pointer, intent(out) :: aKeySet   ! pointer to new KeySet

    integer(kind=TERRK)          :: irc = TYPH_SUCCESS
