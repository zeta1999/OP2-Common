...
SUBROUTINE bres_calc_kernel(opDat1,opDat3,opDat4,opDat5,opDat6,ind_maps1,ind_maps3,ind_maps4,ind_maps5,mappingArray1,ma&
&ppingArray2,mappingArray3,mappingArray4,mappingArray5,ind_sizes,ind_offs,blkmap,offset,nelems,nthrcol,thrcol,blockOffs&
&et,blockID)
IMPLICIT NONE
REAL(kind=8), DIMENSION(0:*) :: opDat1
...
CALL bres_calc_modified(opDat1SharedIndirection(1 + mappingArray1(i1 + threadBlockOffset) * 2:1 + mappingArray1(i1 + th&
&readBlockOffset) * 2 + 2 - 1),opDat1SharedIndirection(1 + mappingArray2(i1 + threadBlockOffset) * 2:1 + mappingArray2(&
&i1 + threadBlockOffset) * 2 + 2 - 1),opDat3SharedIndirection(1 + mappingArray3(i1 + threadBlockOffset) * 4:1 + mapping&
&Array3(i1 + threadBlockOffset) * 4 + 4 - 1),opDat4SharedIndirection(1 + mappingArray4(i1 + threadBlockOffset) * 1),opD&
&at5Local,opDat6((i1 + threadBlockOffset) * 1))
...
DO i2 = 0, 4 - 1, 1
opDat5(i2 + opDat5IndirectionMap(i1 + 1) * 4) = opDat5(i2 + opDat5IndirectionMap(i1 + 1) * 4) + opDat5SharedIndirection&
&(1 + (i2 + i1 * 4))
END DO
...
SUBROUTINE bres_calc_host(userSubroutine,set,opArg1,opArg2,opArg3,opArg4,opArg5,opArg6)
IMPLICIT NONE
...
CALL c_f_pointer(actualPlan_bres_calc%ind_sizes,ind_sizes_bres_calc,(/actualPlan_bres_calc%nblocks * numberOfIndirectOp&
&Dats/))
CALL c_f_pointer(actualPlan_bres_calc%ind_offs,ind_offs_bres_calc,(/actualPlan_bres_calc%nblocks * numberOfIndirectOpDa&
&ts/))
...
DO i2 = 0, nblocks - 1, 1
threadID = omp_get_thread_num()
CALL bres_calc_kernel(opDat1Local,opDat3Local,opDat4Local,opDat5Local,opDat6Local,ind_maps1_bres_calc,ind_maps3_bres_ca&
&lc,ind_maps4_bres_calc,ind_maps5_bres_calc,mappingArray1_bres_calc,mappingArray2_bres_calc,mappingArray3_bres_calc,map&
&pingArray4_bres_calc,mappingArray5_bres_calc,ind_sizes_bres_calc,ind_offs_bres_calc,blkmap_bres_calc,offset_bres_calc,&
&nelems_bres_calc,nthrcol_bres_calc,thrcol_bres_calc,blockOffset,i2)
END DO
...
accumulatorHostTime = endTimeHost - startTimeHost
loopTimeHostbres_calc_3019043301 = loopTimeHostbres_calc_3019043301 + accumulatorHostTime
returnSetKernelTiming = setKernelTime(2,userSubroutine,accumulatorKernelTime / 1000.00,actualPlan_bres_calc%transfer,ac&
&tualPlan_bres_calc%transfer2)
END SUBROUTINE
...
CALL c_f_pointer(actualPlan_res_calc%ind_sizes,ind_sizes_res_calc,(/actualPlan_res_calc%nblocks * numberOfIndirectOpDat&
&s/))
...
DO i2 = 0, nblocks - 1, 1
threadID = omp_get_thread_num()
CALL res_calc_kernel(opDat1Local,opDat3Local,opDat5Local,opDat7Local,ind_maps1_res_calc,ind_maps3_res_calc,ind_maps5_re&
&s_calc,ind_maps7_res_calc,mappingArray1_res_calc,mappingArray2_res_calc,mappingArray3_res_calc,mappingArray4_res_calc,&
&mappingArray5_res_calc,mappingArray6_res_calc,mappingArray7_res_calc,mappingArray8_res_calc,ind_sizes_res_calc,ind_off&
&s_res_calc,blkmap_res_calc,offset_res_calc,nelems_res_calc,nthrcol_res_calc,thrcol_res_calc,blockOffset,i2)
END DO
...
accumulatorHostTime = endTimeHost - startTimeHost
loopTimeHostres_calc_3884922247 = loopTimeHostres_calc_3884922247 + accumulatorHostTime
returnSetKernelTiming = setKernelTime(1,userSubroutine,accumulatorKernelTime / 1000.00,actualPlan_res_calc%transfer,act&
&ualPlan_res_calc%transfer2)
END SUBROUTINE
...
DO i1 = sliceStart, sliceEnd - 1, 1
CALL update_modified(opDat1(i1 * 4:i1 * 4 + 4 - 1),opDat2(i1 * 4:i1 * 4 + 4 - 1),opDat3(i1 * 4:i1 * 4 + 4 - 1),opDat4(i&
&1 * 1),opDat5)
END DO
...
sliceEnd = opSetCore%size * (i1 + 1) / numberOfThreads
threadID = omp_get_thread_num()
CALL update_kernel(opDat1Local,opDat2Local,opDat3Local,opDat4Local,reductionArrayHost5(threadID * 1 + 1),sliceStart,sli&
&ceEnd)
END DO
